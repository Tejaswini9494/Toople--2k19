...
if(isset($search_submit)) {
	...
	if($search_title!='') {
		...
		$qry='';
				...
				$display =6; 
				////======= Paging =====///
$query1= "SELECT resource_id, resource_category, resource_title, resource_contents, resource_link FROM  too_resources  WHERE resource_id!='' AND status='A' AND resource_type='20' $qry ORDER BY resource_id DESC";
...
$query3="select category_id, category_name from master_category where category_for='20' AND status='A' ORDER BY category_id DESC";
...
<h3 class="">How to’s</h3>
<div class="gap30"></div>

<div class="well filterBox">
	<form id="form_search" method="POST" enctype="multipart/form-data">
		<div class="col-md-2 col-sm-6">
				<input type="text" id="search_title" name="search_title" class="form-control" placeholder="Title" value="<?php echo $search_title; ?>">
		</div>
		<div class="col-md-2 col-sm-6">
			<select name="search_cat" id="search_cat" class="form-control">
				<option value="">Tutorial Category</option>
		...
		<div class="col-md-1 col-sm-6">
			...
			<a href="tutorial.php" class="btn submitM cancelBtn">Clear</a>
		</div>
...
<?php $i=1; while($statement2->fetch()) { ?>
	...
	<div class="col-sm-6">
		<div class="tutorialBox">
			<a href="tutorial_view.php?id=<?php echo $resource_id; ?>"><?php echo $resource_title; ?></a>
			<div class="gap5"></div>
			<h4><?php echo getValue('master_category', 'category_name', 'category_id', $resource_category); ?></h4>
			<div class="gap5"></div>
			<p>
				<?php echo substr($resource_contents, 0, 350)."..."; ?>
			</p>
		</div>
	</div>
	...
	<ul class="pagination">
			...
			if($_SERVER["QUERY_STRING"]!=""){$qs=explode("&",$_SERVER["QUERY_STRING"]);}
				...
				if ($startrow != 0) {  
					...
					print("<li><a href='tutorial.php?$qstr&startrow=$prevrow' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a> </li>");
				} 
				...
				if ($pages > 1) {
					for ($i=1; $i <= $pages; $i++) { 
						...
						if ($presPage==$i){
							...
							print("<li><a href='tutorial.php?$qstr&startrow=$nextrow'>$i </a></li>");
						}
					} 
				}
				...
				if ($endrow	< $numRows){
					if ($pages != 1){
						...
						echo("<li><a href='tutorial.php?$qstr&startrow=$nextrow' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>");
					}
				}
			?>
		  </ul></nav>
