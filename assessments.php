...
if(isset($search_submit)) {
	...
	if($search_title!='') {
		...
		$qry='';
				...
				$display =5; 
				////======= Paging =====///
$query1= "SELECT resource_id, resource_category, resource_title, resource_contents, resource_link FROM  too_resources  WHERE resource_id!='' AND status='A' AND resource_type='22' $qry ORDER BY resource_id DESC";
...
<div class="well filterBox">
	<form id="form_search" method="POST" enctype="multipart/form-data">
		<div class="col-md-2 col-sm-6">
				<input type="text" id="search_title" name="search_title" class="form-control" placeholder="Assessment Name" value="<?php echo $search_title; ?>">
		</div>
		<div class="col-md-2 col-sm-6">
			<select name="search_cat" id="search_cat" class="form-control">
				<option value="">Assessment Category</option>
		...
		<div class="col-md-1 col-sm-6">
			...
			<a href="assessments.php" class="btn submitM cancelBtn">Clear</a>
		</div>
		<div class="gap1"></div>
	</form>
</div>
...
<div class="assmentBox">
	<a href="assessments_view.php?id=<?php echo $resource_id; ?>"><?php echo $resource_title; ?></a>
	<div class="gap5"></div>
	<h4><?php echo getValue('master_category', 'category_name', 'category_id', $resource_category); ?></h4>
	<div class="gap5"></div>
	<p>
		<?php echo substr($resource_contents, 0, 500)."..."; ?>
	</p>
</div>
<div class="gap30"></div>
<?php $i++; } ?>
	...
	<ul class="pagination">
			...
			if($_SERVER["QUERY_STRING"]!=""){$qs=explode("&",$_SERVER["QUERY_STRING"]);}
				...
				if ($startrow != 0) {  
					...
					print("<li><a href='assessments.php?$qstr&startrow=$prevrow' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a> </li>");
				} 
				...
				if ($pages > 1) {
					for ($i=1; $i <= $pages; $i++) { 
						...
						if ($presPage==$i){
							...
							print("<li><a href='assessments.php?$qstr&startrow=$nextrow'>$i </a></li>");
						}
					} 
				}
				...
				if ($endrow	< $numRows){
					if ($pages != 1){
						...
						echo("<li><a href='assessments.php?$qstr&startrow=$nextrow' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>");
					}
				}
			?>
		  </ul></nav>
