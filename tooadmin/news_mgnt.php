...
for ($i=0; $i <count($_REQUEST['checkbox1']); $i++)
{
	...
	if ($button_action=="Active") {
		$querym = "UPDATE master_news SET status= 'A' WHERE news_id= ?";
		$action="GA";
	}else if($button_action=="Deactive") {
		$querym = "UPDATE master_news SET status= 'D' WHERE news_id= ?";
		$action="GA";
	}else if($button_action=="Delete") {
		$querym="Delete from master_news WHERE news_id= ?";
		$action="GA";
	}
...
{
	$qry='';
	if($search_news!='' || $search_content!='' || $search_status!=''){
	$qry.=" AND news LIKE '%".$search_news."%' AND news_content LIKE '%".$search_content."%' AND status LIKE '%".$search_status."%'";
	}

}else{
$qry='';
}
$query="select * from master_news where news_id!='' $qry ORDER BY news_id DESC";
$statement=$mysqli->prepare($query);
$statement->execute();
$statement->store_result();
$statement->bind_result($news_id,$news,$news_content,$news_image,$created_by,$added_date,$updated_on,$status);
    ...
    <table align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td align="right"> </td>
       		...
       		<td><input name="search_news" id="search_news" class="form-control" placeholder="News" type="text" value="<?php echo $search_news; ?>"/></td>
		<td><input name="search_content" id="search_content" class="form-control" placeholder="News Content" type="text" value="<?php echo $search_content; ?>"/></td>
<td>
...
<table id="dataTable" class="table table-bordered table-striped tabBorder">
  <thead>
    <tr class="tr1">
      ...
      <td><strong>#</strong></td>
      <td><strong><a href="#" class="linkB"> News </a></strong></td>
      <td><strong><a href="#" class="linkB"> News Content </a></strong></td>
      <td><strong><a href="#" class="linkB"> Image</a></strong></td>
...
?>
    <tr class="tr">
      <td align="left"><input type="checkbox" name="checkbox1[]" value="<?php echo $news_id;?> "/></td>
      <td   align="left"><a href="newsEdit.php?id=<?php echo $news_id; ?>"><img src="images/edit.png" alt="edit" /></a> </td>
      <td><?php echo $i;?></td>
      <td><a href="newsView.php?id=<?php echo $news_id; ?>"><?php echo $news; ?></a></td>
      <td><?php echo $news_content; ?></td>
      <td><img src="../uploads/master_news/<?php echo $news_image; ?>" width="100px"></td>
