...
if(isset($delete_id))
{	
	$sql_delete = "DELETE FROM co_agreement WHERE user_id='$id' AND co_agreement_id='$delete_id'";
...
if(isset($edit_id))
{
	$sql_edit = "SELECT ca_agree_no,ca_start_date,ca_end_date,ca_pay_date,ca_pay_amt,ca_note FROM co_agreement where user_id='$id' AND co_agreement_id='$edit_id'";
...
if(isset($save_submit)){
	$query = "UPDATE draw_down_payment_details SET type_of_procurement=?,price=?,unit_of_measure=?,quality=?,draw_down_amount=? where user_id='$id'";
	$statement= $mysqli->prepare($query);  
	$statement->bind_param('sissi',$type_of_procurement,$price,$unit_of_measure,$quality,$draw_down_amount);
	$statement->execute();
	header("location:CISEdit3.php?id=$id");
}
$sql = "SELECT type_of_procurement,price,unit_of_measure,quality,draw_down_amount FROM draw_down_payment_details where user_id='$id'";
...
<div class="formss formss1">
	...
	<div class="tab-content">
		<div class="tab-pane fade in active">
			...
			<!------------>
			<h3>Institution Agreement</h3>
			<div class="gap10"></div>
			<div class="item1">
				<div class="gap20"></div>
			<form id="form_reg-institution3" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						...
						<input type="text" name="ca_start_date" id="ca_start_date" value="<?php echo $start;?>" class="form-control" placeholder="Start Date" >
						</div>
						<div class="gap1"></div>
...
<?php
$sql = "SELECT co_agreement_id,ca_pay_date,ca_pay_amt FROM  co_agreement  where user_id='$id' ORDER BY created_on DESC";
$statement1=$mysqli->prepare($sql);
$statement1->execute();
$statement1->store_result();
...
?>
						<tr>
							...
							<td align="right"><a href="CISEdit3.php?id=<?php echo $id;?> &edit_id=<?php echo $co_agreement_id;?>"><i class="fa fa-pencil edit"></i></a><a id="delete" href="CISEdit3.php?id=<?php echo $id;?> & delete_id=<?php echo $co_agreement_id;?>"><i class="fa fa-trash trash"></i></a></td>
						</tr>
<?php } ?>
					</table>
				</div>
