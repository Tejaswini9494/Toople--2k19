...
if(isset($ins_submit)) {
	...
	if($insid!='') {
		...
		$doi=sysConvert2($doi);
		$query3 = "UPDATE too_internship SET category=?, ins_title=?, company=?, job=?, ecprogram=?, ecyoc=?, ecpercent=?, ecgender=?, compensation=?, techarea=?, certificate=?, tsdate=?, tedate=?, joblocation=?, worktype=?, hireprocess=?, doi=?  WHERE internship_id='$insid'";
		$statement3= $mysqli->prepare($query3);
		$statement3->bind_param('sssssssssssssssss', $category, $ins_title, $company, $job, $ecprogram, $ecyoc, $ecpercent, $ecgender, $compensation, $techarea, $certificate, $tsdate, $tedate, $joblocation, $worktype, $hireprocess, $doi);
		$statement3->execute();
...
<form id="form_ins" method="POST" enctype="multipart/form-data">
	...
	<div class="gap20"></div>
		...
		<div class="well">
			...
			<div class="gap5"></div>
			<input type="text" name="ecpercent" id="ecpercent" class="form-control" value="<?php echo $ecpercent; ?>">
			</div>
			<div class="col-md-3">
			<label>Gender</label>
			<div class="gap5"></div>
			<div class="form-control nopadL">
				<label class="col-md-6"><input type="radio" name="ecgender" id="ecgender_male" <?php if($ecgender=='Male') echo "checked"?> value="Male"> Male</label>
				<label class="col-md-6"><input type="radio" name="ecgender" id="ecgender_female" <?php if($ecgender=='Female') echo "checked"?> value="Female"> Female</label>
			</div>
			</div>
			<div class="gap1"></div>
		</div>
	...
	<div class="gap30"></div>
		...
		<div class="well">
			...
			<select id="techarea" name="techarea" class="form-control">
				<option value="" >Select</option>
			<?php while($statement6->fetch()) { ?>
				<option value="<?php echo $subcategory_id2; ?>" <?php if($subcategory_id2==$techarea) { echo "selected"; } ?>><?php echo $subcategory_name2; ?></option>
			<?php } ?>
			</select>
			</div>
			<div class="col-md-6">
			...
			<select id="certificate" name="certificate" class="form-control">
				<option value="" >Select</option>
			<?php while($statement7->fetch()) { ?>
				<option value="<?php echo $subcategory_id3; ?>" <?php if($subcategory_id3==$certificate) { echo "selected"; } ?>><?php echo $subcategory_name3; ?></option>
			<?php } ?>
			</select>
			</div>
			<div class="gap1"></div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			...
			<input type="text" name="tsdate" id="tsdate" class="form-control dateIcon" value="<?php echo $tsdate; ?>">
			</div>
			<div class="gap1"></div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			<div class="input-group">
			<span class="input-group-addon"><i class="fa fa-clock-o"></i></span>
			<input type="text" name="tedate" id="tedate" class="form-control dateIcon" value="<?php echo $tedate; ?>">
			</div>
			<div class="gap1"></div>
	...
	<div class="form-group">
		...
		<div class="col-sm-9">
			...
			<input type="text" name="doi" id="doi" class="form-control dateIcon" value="<?php echo $doi; ?>">
			</div>
			<div class="gap1"></div>
	...
	<div class="col-sm-9 col-sm-push-3">
		...
		<a href="internship.php" class="btn submitM cancelBtn">Cancel</a>
	</div>
</form>
<div class="gap20"></div>
