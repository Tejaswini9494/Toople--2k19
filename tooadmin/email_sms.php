...
$fm_email_id = "testevince1@gmail.com";
		...
		Click the link below to reset your password using our secure server link below.<br>
                                <a href='".$murl."resetpassword.php?id=".$enc_id."' target='_blank' style='text-decoration:none; color: #F58220;'>".$murl."resetpassword.php?uid=".$enc_id."</a>";
            ...
            if($val1=='10'){
				...
				$user_fgt = $mysqli->prepare("SELECT user_id, user_email,user_pwd FROM too_users WHERE user_id= ? AND user_type= ?");
				$user_fgt->bind_param("i", $val2, $val3);
				$user_fgt->execute();
				$user_fgt->bind_result($user_id, $user_email, $user_pwd);
				$user_fgt->store_result();
				$user_fgt->fetch();
                                $user_fgt->close();
				$user_pwd=decrypt($user_pwd,'tooople#@D2016');
				$enc_id=encrypt($user_id,'tooople#@D2016');
				...
				User-Name:".$user_email. "<br>Password:.".$user_pwd.".";
...
<div style="width: 660px; margin: 0px auto; font-size: 14px; font-family: Arial; color: #555; border:1px solid #E6E6E6;">
	...
	$message .='<div style="padding:20px 30px; background:#E6E6E6; font-size: 13px; box-shadow: 0 1px 5px;">
		...
		//echo $from."<br>".$to."<br>".$emailSubject."<br>".$message;
