...
for ($i=0; $i <count($_REQUEST['checkbox1']); $i++)
{
	...
	if ($button_action=="Active") {
		$querym = "UPDATE master_category SET status= 'A' WHERE category_id= ?";
		$action="GA";
	}else if($button_action=="Deactive") {
		$querym = "UPDATE master_category SET status= 'D' WHERE category_id= ?";
		$action="GA";
	}else if($button_action=="Delete") {
		$querym="Delete from master_category WHERE category_id= ?";
		$action="GA";
	}
...
if(isset($search))
{
	$qry='';
	if($search_for!=''){
	$qry.=" AND category_for='".$search_for."'";
	}
	if($search_category!=''){
	$qry.=" AND category_name LIKE '%".$search_category."%'";
	}
	if($search_status!=''){
...
$query="select * from master_category where category_id!=''$qry ORDER BY category_id DESC";
//echo $query;
$statement=$mysqli->prepare($query);
$statement->execute();
$statement->store_result();
$statement->bind_result($category_id,$category_for,$category_name,$created_by,$added_date,$updated_on,$status);
...
<h1>
<span class="back">
<a href="metaCategoryAdd.php">Add</a>
</span>
 Settings  &raquo;  Meta  &raquo;    Category  &raquo;  View 
</h1>
...
<form name="frm1" id="frm1" action="" method="post" >
    <table align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <td><select name="search_for" id="search_for" class="boxM">
		<option value=''> Select Category For </option>
		<?php categoryFor($search_for);?>
        </select>
        </td>
		<td><input name="search_category" id="search_category" class="form-control" placeholder="Category" type="text" value="<?php echo $search_category; ?>"/></td>
<td>
...
<th width="30" align="center"><input type="checkbox" name="selall3" onclick="return selall1(this);" /></th>
      ...
      <td><strong>#</strong></td>
      <td><strong>  Category for </strong></td>
...
{ 
$nab="select category_list from category_for where category_for='$category_for'";
	$statementt=$mysqli->prepare($nab);
$statementt->execute();
$statementt->store_result();
$statementt->bind_result($category_for_name);
...
?>
    <tr class="tr">
      <td align="left"><input type="checkbox" name="checkbox1[]" value="<?php echo $category_id;?> "/></td>
      <td align="left"><a href="metaCategoryEdit.php?id=<?php echo $category_id; ?>"><i class="fa fa-pencil-square-o" ></i></a></td>
      <td><?php echo $i;?></td>
      <td><?php echo $category_for_name; ?></td>
