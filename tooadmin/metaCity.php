...
for ($i=0; $i <count($_REQUEST['checkbox1']); $i++)
{
	...
	if ($button_action=="Active") {
		$querym = "UPDATE master_city SET status= 'A' WHERE city_id= ?";
		$action="GA";
	}else if($button_action=="Deactive") {
		$querym = "UPDATE master_city SET status= 'D' WHERE city_id= ?";
		$action="GA";
	}else if($button_action=="Delete") {
		$querym="Delete from master_city WHERE city_id= ?";
		$action="GA";
	}
...
{
	$qry='';
	if($search_country!='' || $search_state!='' || $search_city!='' || $search_status!=''){
	$qry.=" AND country_id LIKE '%".$search_country."%' AND state_id LIKE '%".$search_state."%' AND city_name LIKE '%".$search_city."%' AND status LIKE '%".$search_status."%'";
	}

}else{
$qry='';
}
$query="select * from master_city WHERE city_id!=''$qry ORDER BY city_id DESC";
$statement=$mysqli->prepare($query);
$statement->execute();
$statement->store_result();
$statement->bind_result($city_id,$country_id,$state_id,$city_name,$created_by,$added_date,$updated_on,$status);
...
<h1>
<span class="back">
<a href="metaCityAdd.php">Add City</a>
</span>
...
<form name="frm1" id="frm1" action="" method="post" >
 ...
 <td><select name="search_state" id="search_state" class="boxM">
		<option value=''> Select State </option>
        <?php stateId($search_state);?>
        </select>
        </td>
		<td><input name="search_city" id="search_city" class="form-control" placeholder="City" type="text" value="<?php echo $search_city; ?>"/></td>
<td>
  ...
  <thead>
   <tr class="tr1">
    ...
    <td width="20"  align="left" ><i class="fa fa-pencil-square-o"></i></td>
      ...
      <td  ><strong><a href="#" class="linkB"> State </a></strong></td>
      <td  ><strong><a href="#" class="linkB"> City </a></strong></td>
...
?>
    <tr class="tr">
      <td align="left"><input type="checkbox" name="checkbox1[]" value="<?php echo $city_id;?> "/></td>
      <td align="left"><a href="metaCityEdit.php?id=<?php echo $city_id; ?>"><i class="fa fa-pencil-square-o" ></i></a></td>
      <td><?php echo $i;?></td>
      <td><?php echo getCountryName($country_id); ?></td>
      <td><?php echo getStateName($state_id); ?></td>
