...
if(isset($submit_count)) {
	...
	$resource_category=$resource_category1.$resource_category2.$resource_category3;
	//echo $resource_category; exit;

	$status='A';
	$query="insert into too_resources(resource_category, resource_title ,resource_contents, resource_link, resource_type, added_date, status) values(?,?,?,?,?,now(),?)";
	$statement=$mysqli->prepare($query);
	$statement->bind_param('isssss',$resource_category, $resource_title, $resource_contents, $resource_link, $resource_type, $status);
	$statement->execute();
...
<form id="form_resource" method="post" id="" enctype="multipart/form-data" action="">
  <table cellpadding="0" cellspacing="0" align="center">
    ...
    <tr>
      ...
      <td>
		<select name="resource_type" id="resource_type" class="boxM" onchange="load_rescategory(this.value)">
			...
			<option value="20">Tutorial</option>
			<option value="21">Learn Videos</option>
			<option value="22">Assessments</option>
		</select>
	</td>
    </tr>
    ...
    <tr>
      ...
      <td><input type="text" name="resource_title" id="resource_title" placeholder="Enter Resources Title" class="boxM"></td>
    </tr>
    <tr>
      <td align="right"> Contents :<span class="red">*</span></td>
      <td><textarea name="resource_contents" id="resource_contents" placeholder="Enter Resource Contents" class="boxM"></textarea></td>
    </tr>
<!--
    <tr>
...
-->
    <tr id="only21" style="display:none;">
      ...
      <td><textarea name="resource_link" id="resource_link" placeholder="" class="boxM"></textarea></td>
    </tr>
