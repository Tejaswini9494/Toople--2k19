...
for ($i=0; $i <count($_REQUEST['checkbox1']); $i++)
{
	...
	if ($button_action=="Active") {
		$querym = "UPDATE home_video SET status= 'A' WHERE home_video_id= ?";
		$action="GA";
	}else if($button_action=="Deactive") {
		$querym = "UPDATE home_video SET status= 'D' WHERE home_video_id= ?";
		$action="GA";
	}else if($button_action=="Delete") {
		$querym="Delete from home_video WHERE home_video_id= ?";
		$action="GA";
	}
...
if(isset($search))
{
	$qry='';
	if($search_title!=''){
		$qry.=" AND home_video_title LIKE '%".$search_title."%'";
	}
	if($search_status!=''){
...
$query="select home_video_id, home_video_title, status from home_video where home_video_id!='' $qry ORDER BY home_video_id DESC";
$statement=$mysqli->prepare($query);
$statement->execute();
$statement->store_result();
$statement->bind_result($home_video_id, $home_video_title, $status);
...
<h1>
<span class="back">
<a href="homevideos_mgntAdd.php">Add Home Video</a>
</span>
 Home Video Managment
</h1>
...
?>
    <tr class="tr">
      <td align="left"><input type="checkbox" name="checkbox1[]" value="<?php echo $home_video_id;?> "/></td>
      <td   align="left"><a href="homevideos_mgntEdit.php?id=<?php echo $home_video_id; ?>"><img src="images/edit.png" alt="edit" /></a> </td>
      <td><?php echo $i;?></td>
      <td><?php echo $home_video_title; ?></td>
