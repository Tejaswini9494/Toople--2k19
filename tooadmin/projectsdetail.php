...
for ($i=0; $i <count($_REQUEST['checkbox1']); $i++)
{
	...
	$statementm = $mysqli->prepare($querym);
        ...
        header("Location:projectsdetail.php");
...
$sql1="select institution_project_id from institution_project_assign where student_id='$pro_id'";
$stmt1=$mysqli->prepare($sql1);
$stmt1->execute();
$stmt1->store_result();
$stmt1->bind_result($institution_project_id);
while($stmt1->fetch()){

	$sql3="select project_id from institution_project where institution_project_id='$institution_project_id'";
	$stmt3=$mysqli->prepare($sql3);
	$stmt3->execute();
	$stmt3->store_result();
	$stmt3->bind_result($project_id);
	$stmt3->fetch();


	$sql2="select * from too_projects where proj_id='$project_id'";
	$stmt2=$mysqli->prepare($sql2);
	$stmt2->execute();
	$stmt2->store_result();
	$stmt2->bind_result($proj_id,$project_id,$name,$category,$proj_created_by,$proj_status,$creation_date,$termination_date,$proj_abstract,$notes,$duration,$dev_platform,$created_by,$added_date,$updated_on,$status);
	$stmt2->fetch();
if($status=='A'){
$status1='Active';
}
