...
$sql->execute();
$sql->store_result();
$sql->bind_result($proj_id,$project_id,$name,$category,$busi_domain,$proj_method,$proj_created_by,$proj_status,$paid_amt,$creation_date,$termination_date,$proj_abstract,$notes,$duration,$proj_cat,$dev_platform,$proj_img,$created_by,$added_date,$updated_on,$status);
while($sql->fetch())
{ 
...
?>
  <tr class="tr">
	...
	<td>
		<a href="contentTransaction.php?proid=<?php echo $proj_id; ?>" class="submitM">Apply</a>
	</td>
	<td>
		<?php 
			$sql1a="select * from too_project_cost  where proj_id='$proj_id'";
			$sql1=$mysqli->prepare($sql1a); 
			$sql1->execute();
			$sql1->store_result();
			$sql1->fetch();
			/*if($sql1->num_rows() > 0){ 
		...
		<a href="productCost.php?pid=<?php echo $proj_id; ?>&et=Y" class="submitM">Apply</a>
	    <?php //} ?>
	</td>
